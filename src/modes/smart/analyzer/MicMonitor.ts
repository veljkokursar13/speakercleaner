 ...
 * 
 * Uses react-native-audio-record for raw PCM audio access
 * Falls back gracefully in Expo Go where native modules aren't available
 */

import { Platform } from 'react-native';

// Try to import react-native-audio-record, but handle gracefully if not available
let AudioRecord: any = null;
try {
  AudioRecord = require('react-native-audio-record').default;
} catch (error) {
  console.warn('[MicMonitor] react-native-audio-record not available (Expo Go limitation)');
}
...
export class MicMonitor {
  ...
  /**
   * Initialize microphone permissions and audio recording
   */
  async initialize(): Promise<boolean> {
    try {
      if (!AudioRecord) {
        console.log('[MicMonitor] Running in Expo Go - microphone monitoring disabled');
      ...
      const options = {
        ...
        wavFile: 'mic_monitor.wav',
      };

      AudioRecord.init(options);
      
      // Request permissions
      // Note: react-native-audio-record handles permissions internally
      return true;
    } catch (error) {
  ...
  async start(sampleIntervalMs = 100): Promise<void> {
    ...
    if (!AudioRecord) {
      console.log('[MicMonitor] 🔊 Simulating microphone monitoring (Expo Go)');
      this.isActive = true;
      this.snapshots = [];
  ...
  async stop(): Promise<MonitoringReport> {
    ...
    if (AudioRecord) {
      ...
      console.log('[MicMonitor] ⏹️ Stopping simulated monitoring');
    }
  ...
  async getCurrentLevel(): Promise<number> {
    ...
    try {
      // react-native-audio-record doesn't provide direct metering
      // Use the latest snapshot's RMS if available
      if (this.snapshots.length > 0) {
        return this.snapshots[this.snapshots.length - 1].rms;
      }
    } catch {
   ...
   * Note: react-native-audio-record provides raw PCM data via callbacks
   */
  private async getAudioSamples(): Promise<Float32Array | null> {
    // react-native-audio-record provides data via callback
    // For now, use the latest buffer if available
    if (this.audioBuffer.length > 0) {
      const latest = this.audioBuffer[this.audioBuffer.length - 1];
      return latest;
    }
    
    // Fallback: estimate from latest snapshot
    if (this.snapshots.length > 0) {
      const latest = this.snapshots[this.snapshots.length - 1];
      const samples = new Float32Array(this.fftWindowSize);
      const frequency = latest.dominantFreq || 1000;
      const amplitude = latest.rms;
      
      for (let i = 0; i < samples.length; i++) {
        const t = i / this.sampleRate;
        samples[i] = amplitude * Math.sin(2 * Math.PI * frequency * t);
        samples[i] += (Math.random() - 0.5) * amplitude * 0.1;
      }
  ...
  /**
   * Expose PCM-like samples and sample rate for offline analysis
   * Note: Uses synthesized approximation when raw PCM isn't available.
   */
  async getSamplesForAnalysis(): Promise<{ samples: Float32Array; sampleRate: number } | null> {
    const samples = await this.getAudioSamples();
    return samples ? { samples, sampleRate: this.sampleRate } : null;
  }
